...
scp -r pre_salt/* root@$master:/root/ >/dev/null
ssh root@$master 'bash /root/setup_and_build.sh'
if [ $? -ne 0 ]; then echo "Failed build fibonacciGenerator on master"; exit; fi
...
echo "sleep for 20 to let all minions connect"
sleep 20
...
ssh root@$master "salt --hard-crash '*' state.highstate"
if [ $? -ne 0 ]; then
